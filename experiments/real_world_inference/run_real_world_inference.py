...
说明:
- 这是推理与可视化实验，不重新训练模型
- 预处理对齐 denoising 可视化入口：
  使用 extract_features_from_fetal_path(...) 获取一致的 raw_fhr / baseline
"""
...
from ctg_pipeline.utils.pathing import DENOISING_RESULTS_ROOT, REAL_WORLD_RESULTS_ROOT, resolve_repo_path
...
def main() -> None:
    ...
    parser.add_argument(
        "--denoiser_model",
        type=str,
        default=str(DENOISING_RESULTS_ROOT / "multilabel_guided_denoising_hard_pred" / "best_model.pt"),
        help="mask-guided denoiser 模型路径",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=str(REAL_WORLD_RESULTS_ROOT / "real_world_inference"),
        help="输出目录",
    )
    ...
    args.denoiser_model = str(resolve_repo_path(args.denoiser_model))
