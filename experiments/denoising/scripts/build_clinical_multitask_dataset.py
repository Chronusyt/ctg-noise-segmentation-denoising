...
def write_description(output_dir: str, summaries: list[dict], cfg: FeatureConfig) -> None:
    ...
    lines = [
        ...
        "- note: acc/dec labels are event masks rather than waveform traces; morphology visualizations should overlay the mask on clean FHR",
        "",
...
def write_label_definition(summary_dir: str) -> None:
    ...
    lines = [
        ...
        "- acc/dec labels are event masks, not waveform traces; plot them as overlays on clean FHR when inspecting morphology",
        "",
