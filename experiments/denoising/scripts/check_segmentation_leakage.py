...
def main():
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default=str(DENOISING_DATASETS_ROOT / "segmentation_hard"),
        help="segmentation 数据集目录",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=str(DENOISING_RESULTS_ROOT / "segmentation_hard" / "leakage_report.txt"),
        help="泄漏报告输出路径",
    )
    args = parser.parse_args()

    data_dir = str(resolve_repo_path(args.data_dir))
    output_path = str(resolve_repo_path(args.output))
