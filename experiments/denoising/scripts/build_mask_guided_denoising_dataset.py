...
def _infer_default_segmentation_model(experiment_tag: str) -> str:
    return str(DENOISING_RESULTS_ROOT / f"multilabel_segmentation_{experiment_tag}" / "best_model.pt")
...
def main():
    ...
    default_seg_model = str(DENOISING_RESULTS_ROOT / "multilabel_segmentation_hard" / "best_model.pt")
    if args.segmentation_model == default_seg_model and experiment_tag != "hard":
        args.segmentation_model = _infer_default_segmentation_model(experiment_tag)

    args.source_dir = str(resolve_repo_path(args.source_dir))
    args.segmentation_model = str(resolve_repo_path(args.segmentation_model))
    if args.output_dir is None:
        suffix = "pred" if args.mask_source == "pred" else "gt"
        args.output_dir = str(DENOISING_DATASETS_ROOT / f"multilabel_guided_denoising_{experiment_tag}_{suffix}")
    else:
